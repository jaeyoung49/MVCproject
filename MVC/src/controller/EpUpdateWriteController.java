...
public class EpUpdateWriteController implements Controller {

	@Override
	public ModelAndView execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		int a_no = Integer.parseInt(request.getParameter("a_no"));
		ArrayList<EpilogueDTO> EpDto = MvcService.getInstance().epilogueDetail(a_no);
		String name = MvcService.getInstance().findCategoryNameByA_no(a_no);
		ModelAndView mv = new ModelAndView("index.jsp?page=EpUpdateWrite");
